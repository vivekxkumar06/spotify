...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-black text-white px-6 md:px-8 py-3 flex items-center justify-between">
      ...
      <div className="flex md:hidden items-center gap-4">
        <Link
          href="/premium"
          className="bg-[#1ED760] text-black text-xs font-bold px-4 py-2.5 rounded-full hover:scale-105 transition-transform whitespace-nowrap"
        >
          Try Premium
        </Link>
        ...
        <button
          ...
          className="text-white hover:text-[#1ED760] focus:outline-none"
      ...
      {isOpen && (
        <div className="absolute top-full left-0 w-full bg-black border-t border-gray-950 py-6 px-8 flex flex-col gap-6 md:hidden z-40 transition-all duration-300 shadow-2xl">
          <Link
            href="/premium"
            onClick={() => setIsOpen(false)}
            className="text-lg font-bold text-gray-200 hover:text-[#1ED760]"
          >
            Premium plans
          </Link>
          <Link
